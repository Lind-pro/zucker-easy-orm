...
public class BuildParameterQueryOperatorTest {
    ...
    public void test() {
        ...
        Assert.assertArrayEquals(parameter.getColumns().stream().map(InsertColumn::getColumn).toArray(), new Object[]{"id", "name"});
        Assert.assertArrayEquals(parameter.getValues().stream().flatMap(Collection::stream).toArray(), new Object[]{1, "2"});
    }
